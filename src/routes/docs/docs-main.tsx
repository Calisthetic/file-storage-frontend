  ...
  const tiles:TileItemProps[] = [
    {
      ...
      title: "User account",
      text: "Each user has a limit of 10GB of free space. "+
    ...
    },
    {
      ...
      title: "Statistic",
      text: "We provide statistics on activity, use of free disk space, and a pie of all types of files that are stored in the cloud"
    },
    {
      icon: (
      ...
      title: "Security",
      text: "The use of advanced technologies in the field of web application security, such as password hashing, "+
    ...
    },
    {
      ...
      title: "Customize",
      text: "The user interface has great possibilities for personalization. "+
    ...
    },
    {
      ...
      title: "Downloads",
      text: "Using a script for dynamically creating archives, you can download any files, folders, and even your entire disk with one click"
    },
    {
      icon: (
        <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"
      ...
      title: "Privacy",
      text: "Any added file will be available only to you. If desired, you can create various links and share files"
    },
    {
      icon: (
      ...
      title: "Learn more about our code",
      text: "The technologies are described here, API documentation and links to source code are provided",
      link: "/docs/code"
    },
    {
      icon: (
  ...
  return (
    <>
        ...
        <div className="container px-2 sm:px-6 py-10 mx-auto items-center flex flex-col">
          ...
          <div className="grid grid-cols-1 gap-y-4 gap-x-4 mt-8 xl:mt-12 lg:gap-x-8 xl:gap-x-16 md:grid-cols-2 max-w-7xl">
            {tiles.map((item, index) => (
              <TileItem key={index} icon={item.icon} link={item.link} title={item.title} text={item.text}></TileItem>
            ))}
          </div>
        </div>
      </section>
      ...
      <section className="flex justify-center mb-8">
        <Link to="/welcome" className="flex items-center justify-center w-1/2 px-5 py-2 text-sm 
...
const TileItem: FunctionComponent<TileItemProps> = ({icon, title, text, link}:TileItemProps) => {
  return (
    ...
    bg-backgroundSecondLight dark:bg-backgroundDark shadow-tileLight dark:shadow-none">
      ...
      {link ? (<ReadMore to={link}></ReadMore>) : null}
    </div>
  );
}
